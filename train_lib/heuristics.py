...
def pass_conflict_hold(
    train: "Train", prev_block_section: "BlockSection", current_sp: "SchedulePoint"
) -> float:
    ...
    prev_station = (
        prev_block_section.from_station
        if prev_block_section.from_station != current_sp.station
        else prev_block_section.to_station
    )

    succeeding_train_csp = next(
        (sp for sp in succeeding_train.schedule if sp.station == current_sp.station),
        None,
    )
    succeeding_train_psp = next(
        (sp for sp in succeeding_train.schedule if sp.station == prev_station), None
    )
    succeeding_train_nsp = next(
        (
            sp
            for sp in succeeding_train.schedule
            if sp.station != current_sp.station
            and sp.arrival_time > current_sp.arrival_time
        ),
        None,
    )
    ...
    if not succeeding_train_csp:
        ...
        raise Exception(
            f"Succeeding train does not have current station {current_sp.station.stn_code} in its schedule"
        )

    if not succeeding_train_psp:
        print("[CRITICAL] THIS SHOULD NOT HAPPEN.. ")
        raise Exception(
            f"Succeeding train does not have previous station {prev_station.stn_code} in its schedule"
        )

    if not succeeding_train_nsp:
        return 0.0  # no next station, succeeding train terminates here

    succeeding_train_pred_arrival = (
        succeeding_train_psp.actual_departure_time
        + prev_block_section._run_minutes(
            succeeding_train,
            succeeding_train.should_accelerate(
                succeeding_train_csp, succeeding_train_csp.dwell_time
            ),
            succeeding_train.should_decelerate(succeeding_train_csp),
        )
    )

    current_train_csp = next(
        (sp for sp in train.schedule if sp.station == current_sp.station), None
    )
    current_train_nsp = next(
        (
            sp
    ...
    if not current_train_csp:
        print("[CRITICAL] THIS SHOULD NOT HAPPEN.. ")
        raise Exception(
            f"Current train does not have current station {current_sp.station.stn_code} in its schedule"
        )
    ...
    if (
        current_sp.departure_time <= succeeding_train_pred_arrival
        and succeeding_train_csp.layover_time <= 0
    ):
        if train.priority > succeeding_train.priority:
            # current train has lower priority (min heap) than succeeding train, must wait
            wait_time = (
                (succeeding_train_pred_arrival - current_sp.departure_time)
                + next_block_section._headway_mins(train, True)
                + next_block_section._run_minutes(
                    succeeding_train,
                    succeeding_train_csp.dwell_time > 0,
                    succeeding_train.should_decelerate(succeeding_train_nsp),
                )
            )  # since we are stopping at this station
            return wait_time
...
def meet_conflict_hold(
    train: "Train", oncoming_block_section: "BlockSection", current_sp: "SchedulePoint"
) -> float:
    ...
    oncoming_train_nsp = (
        oncoming_train.next_schedule_point(oncoming_train_csp)
        if oncoming_train_csp
        else None
    )
    ...
    if not oncoming_train_csp:
        print("[CRITICAL] THIS SHOULD NOT HAPPEN.. ")
        raise Exception(
            f"Oncoming train does not have current station {current_sp.station.stn_code} in its schedule"
        )

    if not oncoming_train_nsp:
        print("[CRITICAL] THIS SHOULD NOT HAPPEN.. ")
        raise Exception(
            f"Oncoming train does not have next station after {current_sp.station.stn_code} in its schedule"
        )

    oncoming_train_pred_arrival = (
        oncoming_train_csp.actual_departure_time
        + oncoming_block_section._run_minutes(
            oncoming_train,
            oncoming_train.should_accelerate(
                oncoming_train_csp, oncoming_train_csp.dwell_time
            ),
            oncoming_train.should_decelerate(oncoming_train_nsp),
        )
    )

    current_train_csp = train.sp_for_station(current_sp.station)
    current_train_nsp = (
        train.next_schedule_point(current_train_csp) if current_train_csp else None
    )
    ...
    if current_sp.departure_time <= oncoming_train_pred_arrival:
        wait_time = (
            oncoming_train_pred_arrival - current_sp.departure_time
        ) + next_block_section._headway_mins(
            train, True
        )  # since we are stopping at this station
        return wait_time
...
def capacity_conflict_hold(train: "Train", current_sp: "SchedulePoint") -> float:
    ...
    next_scheduled_halt_sp = next(
        (
            sp
            for sp in train.schedule
            if sp.station != current_sp.station
            and sp.arrival_time > current_sp.actual_arrival_time
            and sp.layover_time > 0
        ),
        None,
    )
    ...
    next_station = next(
        (
            sp
            for sp in train.schedule
            if sp != current_sp and sp.arrival_time > current_sp.actual_arrival_time
        ),
        None,
    )
    ...
    track_availability = next(
        (
            t
            for t in next_scheduled_halt_station.tracks
            if t.has_platform and not t.resource.count > 0
        ),
        None,
    )
    ...
    # next_block_section = current_sp.station.get_block_to(next_station)
    earliest_departing_train = find_earliest_departing_train(
        next_scheduled_halt_station, current_sp.departure_time
    )
    if earliest_departing_train:
        ...
        if not is_oncoming_train:
            ...
            other_next_station = next(
                (
                    sp
                    for sp in other_train.schedule
                    if sp.arrival_time >= edt_sp.departure_time
                ),
                None,
            )
            if other_next_station:
                other_next_block = edt_sp.station.get_block_to(
                    other_next_station.station
                )
                if other_next_block == next_block and other_next_block and next_block:
                    ...
                    return max(
                        0.0,
                        (wait_until + safety_buffer) - current_sp.actual_arrival_time,
                    )
...
def find_earliest_departing_train(
    station: "Station", before_time: float
) -> tuple["Train", "SchedulePoint"] | None:
