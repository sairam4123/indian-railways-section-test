...
from .captcha_handlers import async_default_captcha_resolver
...
class ETrainAPIAsync:
    def __init__(
        self,
        phpcookie=None,
        captcha_resolver: Callable[
            [str, list[str], str, str], Awaitable[str]
        ...
        self.session.cookie_jar.update_cookies(
            ({"PHPSESSID": self._phpcookie} if self._phpcookie else {})
        )
        ...
        async with self.session.post(
            ...
            try:
                ...
                if "captcha" in json.get("sscript", {}):
                    ...
                    if not curr_cookie:
                        raise ETrainAPIError(
                            "captcha required but no session cookie found"
                        )
    ...
    async def _update_cookie(self, _curr_cookie: Morsel[str]):
    ...
    async def authenticate(self, json_resp: dict):
        code = json_resp.get("sscript")
        if not code:
            raise ETrainAPIError("captcha required but no captcha data found")

        captcha_soup = bs4.BeautifulSoup(code, "html.parser")
        image = captcha_soup.find("img", attrs={"class": "captchaimage"})
        if not image:
            raise ETrainAPIError("captcha required but no captcha image found")

        error = (
            msg_error.get_text()
            if (
                msg_error := captcha_soup.find(
        ...
        async with self.session.get(BASE_URL + str(image.attrs["src"])) as res:
            res: Response
            if res.status != 200:
        ...
        key = await self.captcha_handler(
            encoded_hash, keys, error, str(CAPTCHA_FOLDER / captcha_file)
        )
        try:
    ...
    async def get_all_trains(
        ...
        stn_code: str,
        stn_name: str,
        config: ETrainAllTrainsConfig = ETrainAllTrainsConfig(),
    ):
        json_resp = await self._request(
            f"/station/{stn_name.replace(' ', '-')}-{stn_code.upper()}/all",
            query={"q": "page"},
            form_data={
                "page": f"/station/{stn_name.replace(' ', '-')}-{stn_code.upper()}/all"
            },
        )
    ...
    async def __aexit__(self, exc_type, exc_val, exc_tb):
        if self._phpcookie:
            ...
            print("DEBUG: No session cookie found, skipping cache write")
            await self.session.close()


# class ETrainClientAsync:
#     def __init__(self, api: ETrainAPIAsync):
#         self.api = api
#         self.stations = {}  # Cache for station data (code -> name mapping)
