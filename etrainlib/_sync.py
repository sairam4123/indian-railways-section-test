...
from .captcha_handlers import default_captcha_handler
...
class ETrainAPISync:
    def __init__(
        ...
        self.session.headers = COMMON_HEADERS  # type: ignore
        if not self._phpcookie:
            print(
                "DEBUG: No session cookie found, proceeding without authentication. Some features may not work."
            )
        else:
            print("DEBUG: Session cookie loaded successfully.")
    ...
    def _request(self, path: str, query={}, form_data={}):
        ...
        try:
            ...
            if "captcha" in json.get("sscript", {}):
                ...
                if not curr_cookie:
                    raise ETrainAPIError("captcha required but no session cookie found")
    ...
    def _update_cookie(self, _curr_cookie: str):
        ...
        self.session.cookies.set("PHPSESSID", self._phpcookie)
        AUTH_CACHE.write_text(self._phpcookie)  # write new token to cache immediately
        return True
    ...
    def authenticate(self, json_resp):
        ...
        res = self.session.get(BASE_URL + str(image.attrs["src"]))
        ...
        if not new_json_resp:
            raise ETrainAPIError("no response received after captcha submission")
    ...
    def close(self):
        ...
        if self._phpcookie:
            print("DEBUG: Saving session cookie to cache.")
